import { useEffect, useMemo, useState } from 'react';
import { normalizeCounty } from '../utils/normalize';
import { pickCountyKey } from '../utils/accessors';

const COLORS = ['#FFEDA0','#FED976','#FEB24C','#FD8D3C','#E31A1C']; // 5 classes
const N = COLORS.length;

function parseNum(v) {
  if (v == null) return null;
  const x = Number(String(v).replace(/[, %]/g, ''));
  return Number.isFinite(x) ? x : null;
}

function yearOf(row) {
  for (const k of Object.keys(row)) {
    if (k && k.trim().toLowerCase() === 'year') return row[k];
  }
  return null;
}

// quantile upper-bounds: returns array of length N with class upper limits
function quantileBreaks(values, k) {
  const v = values.slice().sort((a, b) => a - b);
  if (v.length === 0) return [];
  const q = [];
  for (let i = 1; i <= k; i++) {
    const p = (i * (v.length - 1)) / k;
    const lo = Math.floor(p), hi = Math.ceil(p);
    const t = p - lo;
    const val = lo === hi ? v[lo] : v[lo] * (1 - t) + v[hi] * t;
    q.push(val);
  }
  return q;
}

export function useCountyChoropleth({ fileUrl, metricKey, year }) {
  const [rows, setRows] = useState(null);

  useEffect(() => {
    setRows(null);
    if (!fileUrl) return;
    let cancelled = false;
    fetch(fileUrl)
      .then(r => r.json())
      .then(json => { if (!cancelled) setRows(Array.isArray(json) ? json : []); })
      .catch(() => { if (!cancelled) setRows([]); });
    return () => { cancelled = true; };
  }, [fileUrl]);

  // Build county -> value
  const { valueMap, breaks } = useMemo(() => {
    const map = new Map();
    if (!Array.isArray(rows) || !metricKey) return { valueMap: map, breaks: [] };

    // Optionally filter rows by selected year
    const candidate = rows.filter(r => {
      if (!year) return true;
      const ry = yearOf(r);
      return ry != null && String(ry) === String(year);
    });

    // If user didn’t choose a year and file has years, pick the latest per county
    const grouped = new Map(); // county -> rows[]
    for (const r of candidate) {
      const countyName = normalizeCounty(pickCountyKey(r) || '');
      if (!countyName) continue;
      if (!grouped.has(countyName)) grouped.set(countyName, []);
      grouped.get(countyName).push(r);
    }

    // choose row per county
    for (const [name, arr] of grouped.entries()) {
      let row = arr[0];
      if (!year) {
        // pick max year when not filtered
        let maxY = -Infinity;
        for (const r of arr) {
          const y = parseNum(yearOf(r));
          if (y != null && y > maxY) { maxY = y; row = r; }
        }
      }
      const v = parseNum(row?.[metricKey]);
      if (v != null) map.set(name, v);
    }

    const vals = Array.from(map.values());
    const br = vals.length ? quantileBreaks(vals, N) : [];
    return { valueMap: map, breaks: br };
  }, [rows, metricKey, year]);

  function getFill(feature) {
    const name = normalizeCounty(
      feature.properties?.COUNTY || feature.properties?.NAME || ''
    );
    const v = valueMap.get(name);
    if (v == null || !breaks.length) return '#cccccc';
    let i = breaks.findIndex(b => v <= b);
    if (i < 0) i = COLORS.length - 1;
    return COLORS[i];
  }

  return {
    getFill,
    hasData: valueMap.size > 0,
    legend: { breaks, colors: COLORS, title: metricKey || 'Value' },
  };
}
